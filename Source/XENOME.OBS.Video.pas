...
uses
  ...
  System.Contnrs, System.SyncObjs, XENOME.OBS.Types, XENOME.OBS.VideoScaler;

type
  POBSVideoData = ^TOBSVideoData;
  POBSCachedFrameInfo = ^TOBSCachedFrameInfo;
  POBSVideoOutputInfo = ^TOBSVideoOutputInfo;
  POBSVideoInput = ^TOBSVideoInput;
  POBSVideoOutput = ^TOBSVideoOutput;
  POBSVideoScaleInfo = ^TOBSVideoScaleInfo;
  POBSVideoInfo = ^TOBSVideoInfo;

  TOBSVideoInputCallback = procedure(Param: Pointer; Frame: POBSVideoData) cdecl;

  TOBSVideoFrame = record
    Data: Array[0..MAX_AV_PLANES-1] of PAnsiChar;
  ...
  TOBSVideoInput = record
    Conversion: TOBSVideoScaleInfo;
    Scaler: POBSVideoScaler;
    Frame: Array[0..MAX_CONVERT_BUFFERS-1] of TOBSVideoFrame;
    CurFrame: Integer;
    FramerateDivisor: Cardinal;
    FramerateDivisorCounter: Cardinal;
    Callback: TOBSVideoInputCallback;
    Param: Pointer;
  end;
  ...
  TOBSVideoOutput = record // You should NOT use this directly, only translated for debugging purposes!
    ...
    InputMutex: Pointer;
    Inputs: TOBSDArray<TOBSVideoInput>;
